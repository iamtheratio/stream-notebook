...
class NotesService extends BaseService {
    constructor(wss, opts = {}) {
        ...
        // Permissions (overridable via config.json -> notes)
     ...
     * toggling "VIPs can add notes" takes effect without a restart.
     */
    _loadConfig() {
    ...
    _saveState() {
        try {
            fs.writeFileSync(STATE_PATH, JSON.stringify({ enabled: this.enabled, visible: this.visible }, null, 2));
        } catch (err) {
    ...
    /** broadcaster/mod → 'full', VIP (if enabled) → 'add', everyone else → 'view'. */
    _permission(data) {
        if (data.isBroadcaster === true || data.isModerator === true) return 'full';
        if (this.vipCanAdd && data.isVip === true) return 'add';
        return 'view'; // regular chatters: read-only (show / hide / list)
    }
    ...
    _dispatch(rest, perm, user, emoteMap) {
        ...
        // ── Everything below MUTATES notes → broadcaster/mod only (VIP: add) ──
