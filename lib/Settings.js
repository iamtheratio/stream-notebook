...
const DEFAULTS = {
    ...
    // Overlay placement, one set per layout. A single set can't work: the same
    // overlay.html feeds every OBS browser source, so a horizontal and a vertical
    // overlay usually run at the same time and want different answers.
    // overlay.html picks a set from ?type= and applies it over the notebook
    // module rather than editing it — notebook-overlay.js is shared with the
    // upstream project and has to stay byte-identical.
    overlayScale: 1,               // 0.75 | 1 | 1.25 | 1.5 | 2
    overlayCorner: 'bottom-left',  // bottom-left | bottom-right | top-left | top-right
    verticalScale: 2,              // vertical canvases are narrow — bigger by default
    verticalCorner: 'top-left',
